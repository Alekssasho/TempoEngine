...
{
	// TODO: This needs to be rethinked due to not having the index of global entity iteration
	// which is used to access additional arrays
	// It is possible to not have additional arrays but have them inside a component, but then will
	// need to access cross entities
	...
	//TaskGraph::TaskHandle initializeDataJob = graph.AddTask(
	...
	//				const glm::vec3 forward = glm::rotate(rotation, sForwardDirection);
	...
	//// Steer Job
	//TaskGraph::TaskHandle steerJobHandle = graph.AddTask(
	//	"BoidsSystem::Steer",
	//	new Task::ParallelQueryEach(
	//		&m_Query,
