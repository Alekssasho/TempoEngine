...
use data_definition_generated::{
    Level, LevelArgs, LEVEL_EXTENSION, LEVEL_IDENTIFIER,
    GEOMETRY_DATABASE_EXTENSION, GeometryDatabaseArgs, GeometryDatabase, GEOMETRY_DATABASE_IDENTIFIER,
    MeshMapping, MeshMappingArgs,
};
use flecs_rs::*;
use std::io::prelude::*;
...
fn create_mesh_entity(state: &FlecsState, name: &str, pos: glm_vec3, mesh_index: u32) -> (ecs_entity_t, CString) {
    let transform = Components::Transform(Tempest_Components_Transform {
        Position: pos,
        Heading: glm::vec3(1.0, 0.0, 0.0),
    });
    let static_mesh = Components::StaticMesh(Tempest_Components_StaticMesh{
        Mesh: mesh_index,
    });
...
fn geometry_database_export(filename: &str, opt: &CommandLineOptions) -> () {
    let mut builder = flatbuffers::FlatBufferBuilder::new_with_capacity(1024 * 1024);
    {
        ...
        let vertex_buffer = vec!(
            ...
            -0.5f32, 0.0f32, 0.0f32,
            1.0f32, -0.5f32, 1.0f32,
            0.5f32, 0.0f32, 1.0f32,
            -0.5f32, 0.0f32, 1.0f32,
        );
